...
/*   Updated: 2024/12/06 17:43:26 by jsommet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_wall(t_cub *cub, int x, int y)
{
	...
	if (cub->map.raw[y][x] == '1')
		draw_square(&cub->minimap, texcoord, 10, cub->map.col_ceil | 0x101010);
}
	...
	while (++y < 12)
	{
		...
		while (++x < 12)
		{
			...
			if (cub->player.pos.x + x - 6 < 0 || cub->player.pos.x + x - 6 >= cub->map.width)
				continue ;
			draw_wall(cub, cub->player.pos.x + x - 6, cub->player.pos.z + y - 6);
		}
	}
}
