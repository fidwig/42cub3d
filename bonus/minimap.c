...
/*   Updated: 2024/11/26 14:37:58 by jsommet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub.h"

void	draw_square(t_image *img, t_vec3 pos, int s)
{
	int	i;
	int	j;
...
void	draw_minimap(t_cub *cub)
{
	...
	draw_square(&cub->minimap, (t_vec3){48, 48, 0}, 3);
	draw_square(&cub->minimap, (t_vec3){49 + cos(cub->player.rot) * 5.0, 49 + sin(cub->player.rot) * 5.0, 0}, 2);
	y = -1;
	while (++y < 12)
	{
		x = -1;
		while (++x < 12)
		{
			...
			if (cub->map.raw[(int)cub->player.pos.z + y - 6][(int)cub->player.pos.x + x - 6] == '1')
				draw_square(&cub->minimap, (t_vec3){(x - 1 - offset.x) * 10, (y - 1 - offset.y) * 10, 0}, 10);
		}
	}
}
