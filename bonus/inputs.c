...
void	move(t_cub *cub, int x, int y)
{
	...
	mangle = wrap_angle(cub->player.yaw + atan2(x, y));
...
// move.x = x * cos(cub->player.yaw + (M_PI / 2.0)) + y * cos(cub->player.yaw);
// move.z = x * sin(cub->player.yaw + (M_PI / 2.0)) + y * sin(cub->player.yaw);
